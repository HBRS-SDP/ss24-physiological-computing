...
bool RobotManager::thread_init() {
    add_loop_thread([this] { self_loop(); });
    add_loop_thread([this] { input_loop(); });
    return true;
}
...
void RobotManager::process(const std::string &input) {
    ...
    if (command == "exit") {
        ...
        if (!get_functions(input_vector)) {
            const auto get_input = combine_string(input_vector, 1);
...
bool RobotManager::set_functions(const std::vector<std::string> &input) const {
    if (input.size() < 2) {
        return false;
    }
...
void RobotManager::self_loop() const {
    robot->robot_loop();
};

void RobotManager::input_loop() {
    if (std::string command; robot->get_robot_command(command)) {
